...
export const FormMovie = ({ handleAddMovie, handleUpdateMovie, setMovie, movie }) => {
  const [genres, setGenres] = useState([]);

  const getGenres = async () => {
    try {
      let response = await fetch(
        `${import.meta.env.VITE_APP_API_URLBASE}/genres`
      );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      /* 
      >>> EN EL CASO QUE QUERAMOS SUBIR IMAGENES 
      const data = new FormData()
      data.append("title", values.title)
      data.append("rating", values.rating)
      data.append("awards", values.awards)
      data.append("release_data", values.release_data)
